...
const ThemeSelector = ({
  ...
  useEffect(() => {
    // measure once on mount (and after ref available)
    updatedBaseWidth();
    ...
    <div className="max-w-7xl mx-auto px-4">
     ...
     p-4 sm:p-6 bg-gradient-to-r from-white to-violet-50 rounded-2xl border border-violet-100 "
      >
        ...
        <button
          className="w-full flex sm:w-auto items-center justify-between  px-6 py-3 bg-gradient-to-r from-violet-600 
        to-fuchsia-500 text-white font-black rounded-2xl hover:scale-105 transition-all shadow-lg hover:shadow-xl "
          onClick={handleThemeSelection}
        >
          <Check size={18} /><span>Apply Changes</span> 
        </button>
      </div>
